...
{
    // FONDO
    dibujarArena(ARENA_MARGEN_X, ARENA_MARGEN_Y,
        ZONA_DE_COMBATE_X, ZONA_DE_COMBATE_Y,
        0.1f, 0.2f, 0.6f, -5.0f);
    ...
    // ATAQUES — usa getAtaqueObjeto() en vez de los getters viejos
    for (int i = 0; i < 2; i++) {
        const Ataque* ataque = arena->getAtaqueObjeto(i);
    ...
    // COMBATIENTES — glEnable fuera del bucle para que ambos se vean
    glDisable(GL_DEPTH_TEST);
    for (int i = 0; i < 2; i++) {
        if (arena->isVivo(i)) {
            const Animal* c = arena->getCombatiente(i);
            if (c) dibujar(c);   // polimórfico: usa especie_ para elegir sprite
        }
    }
